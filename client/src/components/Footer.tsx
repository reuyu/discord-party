          ...
          >
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
              <h3 style={{ fontSize: '1.2rem', fontWeight: 800, color: '#FFF', display: 'flex', alignItems: 'center', gap: '8px' }}>
                {activeModal === 'privacy' && <><ShieldCheck size={20} color="#10B981" /> {isKo ? '개인정보처리방침 (Privacy Policy)' : 'Privacy Policy'}</>}
                {activeModal === 'terms' && <><FileText size={20} color="#6366F1" /> {isKo ? '서비스 이용약관 (Terms of Service)' : 'Terms of Service'}</>}
                {activeModal === 'about' && <><HelpCircle size={20} color="#F59E0B" /> {isKo ? 'PartyHub 서비스 소개' : 'About PartyHub'}</>}
                {activeModal === 'contact' && <><Mail size={20} color="#EC4899" /> {isKo ? '고객 문의 및 버그 제보' : 'Contact & Feedback'}</>}
              </h3>
              <button
                className="btn-close"
                onClick={() => setActiveModal(null)}
            ...
            <div style={{ fontSize: '0.88rem', color: '#CBD5E1', display: 'flex', flexDirection: 'column', gap: '12px' }}>
              {activeModal === 'privacy' && (
                isKo ? (
                  ...
                  <>
                    <p><strong>1. Personal Information Collected</strong></p>
                    <p>PartyHub requires no user registration. Only the temporary nickname and avatar emoji entered by the user upon joining a room are utilized for temporary in-game identification. No personal identification data is stored or maintained after session disconnection.</p>
                    <p><strong>2. Cookies and Local Storage</strong></p>
                    <p>For convenience, browser LocalStorage may retain your most recent nickname and game play preferences. Users can reset or clear these at any time via browser cache management.</p>
              ...
              {activeModal === 'terms' && (
                isKo ? (
                  ...
                  <>
                    ...
                    <p>These terms govern the terms of use for PartyHub (the "Service") providing real-time party games and web activities.</p>
                    <p><strong>Article 2 (User Obligations)</strong></p>
                    <p>Users must not engage in the following conduct:
                      <br />- Using offensive, violent, or defamatory nicknames
                      <br />- Generating artificial bot traffic, packet tampering, or excessive server load
                      <br />- Malicious spamming or deliberate disruption of gameplay
                    </p>
                    <p><strong>Article 3 (Service Modifications)</strong></p>
                    <p>The Service is provided free of charge and may be updated or temporarily suspended during maintenance or infrastructure migrations.</p>
                    <p><strong>Article 4 (Limitation of Liability)</strong></p>
                    <p>The Service is not liable for interpersonal disputes arising within user groups or external Discord voice channels.</p>
                  </>
                )
              )}
              ...
              {activeModal === 'about' && (
                isKo ? (
                  ...
                  <>
                    <p>PartyHub is a zero-install, link-based party gaming platform created with the motto <strong>"Gather instantly and laugh together with a single link."</strong></p>
                    <p>💡 <strong>Key Features:</strong></p>
                    <ul style={{ paddingLeft: '20px' }}>
                      <li><strong>Zero Installation:</strong> Create a room and share the link via Discord for immediate group access.</li>
                      <li><strong>20+ Exciting Game Modes:</strong> Social deduction (Liar Game, Mafia), reflex duels (High Noon, Snake Royale, CPS Clash), trivia, and party debates.</li>
                      <li><strong>Custom Content Packs:</strong> Create, share, and play custom word packs, trivia decks, and prompt collections with your friends.</li>
                    </ul>
                  </>
                )
              )}
